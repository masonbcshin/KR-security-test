...
def main():
    ...
    symbols = bt.KOREAN_FIXED + [bt.FUT_GOLD, ETN, bt.SPOT_GOLD]
    data = {s: bt.download_one(s, "2010-01-01") for s in symbols}

    validation = proxy_validation(data)
    (OUT / "proxy_validation.json").write_text(json.dumps(validation, indent=2), encoding="utf-8")
    ...
    for exe in bt.EXECUTIONS:
        for gold in (bt.FUT_GOLD, ETN):
            ...
            for cost in bt.COSTS:
                ...
                m = bt.stats(sim)
                m.update(candidate=labels[gold], gold_symbol=gold, cost_bp=cost, execution=exe)
                metrics.append(m)
    ...
    def row(label: str, cost=10.0, exe="next_open"):
        z = df[(df.candidate == label) & (df.cost_bp == cost) & (df.execution == exe)]
        if len(z) != 1:
    ...
    fc, sc = row("G_FUTURES_H", 10.0, "next_close"), row("G_KRX_SPOT_ETN_PROXY", 10.0, "next_close")
    ...
    checks = {
        ...
        "mdd_spot_proxy_no_more_than_10pct_worse": abs(float(s10.mdd)) <= 1.10 * abs(float(f10.mdd)),
    ...
    verdict = {
        ...
        "primary": {
            "G_FUTURES_H": {k: float(f10[k]) for k in ("cagr", "sharpe", "mdd", "calmar", "annual_turnover")},
            "G_KRX_SPOT_ETN_PROXY": {k: float(s10[k]) for k in ("cagr", "sharpe", "mdd", "calmar", "annual_turnover")},
        },
    }
    ...
    report = [
        ...
        "",
        primary[["candidate", "months", "start", "end", "cagr", "sharpe", "mdd", "calmar", "annual_turnover"]].to_markdown(index=False),
        "",
